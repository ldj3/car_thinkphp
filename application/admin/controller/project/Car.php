...
class Car extends Backend
{
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $k => $v) {
              $min_price1 = Db::table('ws_products_sku')->field('price')->where('product_id','=',$v['id'])->select();
              $price = array_column($min_price1, 'price');
              $v['min_price'] = min($price);
              $v['max_price'] = max($price);
              $shop_id = $v['shop_id'];
